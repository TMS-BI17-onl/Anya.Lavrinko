...
DROP TABLE IF EXISTS DimClients CREATE TABLE
	DimClients(
		...
		Passport    UNIQUEIDENTIFIER NOT NULL,--надо ли прописывать это отдельно в констрейнт,что не 0??
		PhoneNumber NVARCHAR(20),
		Address     NVARCHAR(150),
		BirthDate   DATE,
		CONSTRAINT PK_ClientId_DimClients PRIMARY KEY (ClientId),
	    CONSTRAINT UQ_Passport_DimClients UNIQUE (Passport)
		--тут и в табл Менеджеры в структуре вижу Keys (Pk,UQ), Constraints пустые - так и должно быть?
       );
...
DROP TABLE IF EXISTS DimFctSales CREATE TABLE
	DimFctSales(
		...
		CONSTRAINT FK_ClientId_FctSales FOREIGN KEY (ClientID) 
